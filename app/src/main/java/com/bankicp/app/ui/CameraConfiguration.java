...
public class CameraConfiguration {
	...
	public void config(Camera camera) {
		...
		Log.d(TAG, "screen resolution " + screenResolution.x + "*"
				+ screenResolution.y);
	...
	public void initCameraResolution() {
		...
		if (afterResolutions != null
				&& (cameraResolution.x != afterResolutions.width || cameraResolution.y != afterResolutions.height)) {
			Log.w(TAG, "Camera said it supported preview resolution "
					+ cameraResolution.x + 'x' + cameraResolution.y
					+ ", but after setting it, preview resolution is "
					+ afterResolutions.width + 'x' + afterResolutions.height);
		}
	}

	/**
	 * 初始化聚焦模式 即使是同一个手机，前置和后置摄像头的聚焦模式可能不同，如果强行设置可能会出现以下异常： Failed substring
	 * capabilities check, unsupported parameter: 'auto', original: fixed
	 */
	public void initFocusMode() {
		List<String> supportedFocusModes = cameraParameters
				.getSupportedFocusModes();
		String focusMode = findSettableValue(supportedFocusModes,
	...
	public void initPictureResolution() {
		...
		cameraParameters.setPictureSize(bestPictureResolution.x,
				bestPictureResolution.y);
		Log.d(TAG, "set the picture resolution " + bestPictureResolution.x
				+ "x" + bestPictureResolution.y);
		camera.setParameters(cameraParameters);

		Camera.Size afterPicResolution = camera.getParameters()
				.getPictureSize();
		if (null != afterPicResolution
				&& (afterPicResolution.width != bestPictureResolution.x && afterPicResolution.height != bestPictureResolution.y)) {
			Log.w(TAG, "camera support the picture resolution "
					+ bestPictureResolution.x + "x" + bestPictureResolution.y
					+ ", but after set, the picture resolution is "
					+ bestPictureResolution.x + "x" + bestPictureResolution.y);
		}
	}
		...
		if (!pictureStoreDir.exists()) {
			if (!pictureStoreDir.mkdirs()) {
				throw new Exception("failed to create directory: "
						+ pictureStoreDir.getAbsolutePath());
			}
		}
	...
	private Point findBestPreviewResolution() {
		Camera.Size defaultPreviewResolution = cameraParameters
				.getPreviewSize();
		Log.d(TAG, "camera default resolution "
				+ defaultPreviewResolution.width + "x"
		...
		if (rawSupportedSizes == null) {
			...
			return new Point(defaultPreviewResolution.width,
					defaultPreviewResolution.height);
		}
		...
		if (!supportedPreviewResolutions.isEmpty()) {
			...
			Log.d(TAG, "using largest suitable preview resolution: "
					+ largestSize);
			return largestSize;
		}

		// 没有找到合适的，就返回默认的
		Point defaultResolution = new Point(defaultPreviewResolution.width,
				defaultPreviewResolution.height);
		Log.d(TAG, "No suitable preview resolutions, using default: "
				+ defaultResolution);
	...
	/**
	 * 找出最佳照片分辨率 该方法的逻辑与findBestPreviewResolution思路很相似，但还是有部分逻辑不通，分开写的好
	 * 
	 * @return
	 */
	private Point findBestPictureResolution() {
		...
		if (!sortedSupportedPicResolutions.isEmpty()) {
			...
			Point largestSize = new Point(largestPreview.width,
					largestPreview.height);
			Log.d(TAG, "using largest suitable picture resolution: "
					+ largestSize);
			return largestSize;
		}

		// 没有找到合适的，就返回默认的
		Point defaultResolution = new Point(defaultPictureResolution.width,
				defaultPictureResolution.height);
		Log.d(TAG, "No suitable picture resolutions, using default: "
				+ defaultResolution);
